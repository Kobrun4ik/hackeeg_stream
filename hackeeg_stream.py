            ...
            while ((sample_counter < self.max_samples and not self.continuous_mode) or \
            	...
            	for channel_number, sample in enumerate(channel_data):
            		...
            		if channel_number == 0:
            			sample1 = sample
            		if channel_number == 1:
            			pred_samples.append(sample1 - sample )
            			print(f"{sample1 - sample}", file=f)
            	if sample_counter >6 and (pred_samples[sample_counter-2] - pred_samples[sample_counter - 7]) > 55000 and (pred_samples[sample_counter-2] - pred_samples[sample_counter - 7]) < 180000 and (pred_samples[sample_counter -2] -  pred_samples[sample_counter]) > 86000 and (pred_samples[sample_counter - 2] - pred_samples[sample_counter]) < 230000:
